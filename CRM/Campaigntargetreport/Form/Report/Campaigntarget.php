 ...
 *
 */
class CRM_Campaigntargetreport_Form_Report_Campaigntarget extends CRM_Report_Form {
  ...
  $config = CRM_Core_Config::singleton();
    ...
    $this->_columns = array(
      ...
      array('dao' => 'CRM_Contribute_DAO_Contribution',
        ...
        'fields' => 
        array('goal_revenue' => array('title' => ts('Campaigns Target'), 'type' => CRM_Utils_Type::T_MONEY),),
        'grouping' => 'contri-fields',
      ...
      array(
        ...
        array(
          ...
          'total_surplus' =>
          array('title' => ts('Contributions Surplus'),
            'type' => CRM_Report_Form::OP_INT,
            'dbAlias' => 'civicrm_contribution_total_amount_surplus',
            'having' => TRUE,
          ),
        ),
        'group_bys' =>
                ...
                switch (strtolower($stat)) {
                  case 'sum':
                   ...
                   // $this->_statFields[] = "{$tableName}_{$fieldName}_{$stat}";
                    break;                 
                 case 'surplus':                    
                    $select[] = "CASE WHEN SUM(contribution_civireport.total_amount) -  campaign_civireport.goal_revenue <= 0 THEN 0 ELSE SUM(contribution_civireport.total_amount) -  campaign_civireport.goal_revenue  END as civicrm_contribution_total_amount_surplus"; 
                    $this->_columnHeaders["{$tableName}_{$fieldName}_{$stat}"]['type'] = $field['type'];
                    $this->_columnHeaders["{$tableName}_{$fieldName}_{$stat}"]['title'] = $label; 
                   // $this->_statFields[] = "{$tableName}_{$fieldName}_{$stat}"; 
                    break;
    ...
    $this->_from = "
        FROM civicrm_contact  {$this->_aliases['civicrm_contact']} {$this->_aclFrom}
             ...
             LEFT  JOIN civicrm_campaign  {$this->_aliases['civicrm_campaign']}
                     ON {$this->_aliases['civicrm_contribution']}.campaign_id ={$this->_aliases['civicrm_campaign']}.id
  ...
  function statistics(&$rows) {
    ...
    $totalAmount = $average = $softotalAmount = $softAverage  = array();
